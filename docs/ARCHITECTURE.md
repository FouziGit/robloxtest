...
- Aucune boucle par joueur. Chaque service qui a besoin d'un tick ouvre **un seul** `Heartbeat`, jamais un par joueur : `CombatService`, `MovementService`, `MatchService`, `MatchmakingService`, `QuestService`, `ShopService`, `LeaderboardService`, `WorldBossService` (avec accumulateur) et `VfxBroadcaster` (qui s'en sert seulement pour invalider son instantané de destinataires une fois par image), soit neuf au total. Côté client : `VfxTimeline` (toutes les couches de tous les effets), `WorldLighting` (retour immédiat sans pulsation), `FootprintController` (les empreintes de tous les personnages, D-88), et `UI/Motion`, ouverte quand quelque chose bouge et refermée quand plus rien ne bouge.
