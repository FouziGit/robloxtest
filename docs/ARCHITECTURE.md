...
- Aucune boucle par joueur. Chaque service qui a besoin d'un tick ouvre **un seul** `Heartbeat`, jamais un par joueur : `CombatService`, `MovementService`, `MatchService`, `MatchmakingService`, `QuestService`, `ShopService`, `LeaderboardService`, `WorldBossService` (avec accumulateur) et `VfxBroadcaster` (qui s'en sert seulement pour invalider son instantané de destinataires une fois par image), soit neuf au total. Côté client, huit, et `tests/Loops.spec.luau` les déclare toutes avec leur raison : `Feel` (la caméra, seul `BindToRenderStep` du jeu), `VfxTimeline` (toutes les couches de tous les effets), `WorldLighting` (retour immédiat sans pulsation), `FootprintController` (les empreintes de tous les personnages, D-88), `Posture` (les articulations de tous les personnages), `CosmeticController` (les cosmétiques portés), `QualityController` (les images par seconde dont il tire le niveau, D-92), `LockOnController` (le personnage tourné vers l'adversaire verrouillé, D-115), plus `UI/Motion`, ouverte quand quelque chose bouge et refermée quand plus rien ne bouge.
