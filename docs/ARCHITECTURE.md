...
|---|---|
| `RemoteRegistry` | `create(reporter) -> {on(name, handler), setCallback(name, fn), fire(name, player, ...), fireAll(name, ...), fireExcept(name, player, ...), get(name), removePlayer(player)}` |
...
| `AntiCheatService` | `strike(player, source, reason)` (kick après `GameConfig.Server.AbuseKickThreshold` dans `AbuseWindowSeconds`), `Init`, `Start` |
...
`Bootstrap.client.luau` : `Init()` puis `Start()` de chaque contrôleur dans l'ordre : `ClientData`, `Localize`, `SoundController`, `VfxController`, `HudController`, `MenuController`, `InputController`, `ComboController`, `CombatController`, `MovementController`.
...
| `Panel` | `Title`, `Size`, `Closable` → cadre centré avec `UIAspectRatioConstraint` optionnel |
