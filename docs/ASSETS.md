...
son résultat, et `scripts/check.sh` régénère et compare les octets à chaque build. La licence de chaque
fichier est donc celle du dépôt, et sa provenance est le script.
...
|---|---|---|
| `{cinnabar,indigo,umber,verdigris,orpiment}_{attack,body,tail,impact}.wav` (20) | `pigments.py` | les quatre voix de chaque pigment, nommées par les timelines |
