...
Quality gates (`scripts/check.sh`) : `stylua --check src tests` · `selene src tests` · `luau-lsp analyze` strict sur `src/shared` · `lune run tests/run` · `rojo build default.project.json -o build/JutsuBattlegrounds.rbxl`.
...
src/
  shared/                       → ReplicatedStorage/Shared
    Config/                     GameConfig, ElementConfig, JutsuConfig, CombatConfig, ProgressionConfig,
    ...
    Types.luau                  types partagés (Profile, JutsuDef, MatchState, …)
  ...
  server/                       → ServerScriptService
    ...
    Services/                   DataService, SettingsService, ProgressionService, CurrencyService,
                                CombatService, JutsuService, MovementService, VfxBroadcaster,
...
3. `shared/Config/*` : éléments ré-identifiés en anglais (`Fire, Water, Earth, Wind`, `Lightning` réservé), touches par défaut hors déplacement QWERTY/AZERTY (`J K L ;`→ décision : `J, K, L, H` clavier ; voir InputConfig), constantes d'effets sorties de `JutsuEffects` vers `JutsuConfig`.
...
6. `server/Vendor/ProfileStore.luau` (copie officielle épinglée) ; `DataService` avec schéma `DataVersion = 2`, `Pure/DataMigration.luau` (v1 → v2 : `Feu/Eau/Terre/Vent` → `Fire/Water/Earth/Wind` dans Keybinds, IDs de jutsus/cosmétiques FR → EN, `ClaimedRewards` → `Battlepass[SeasonId].Claimed`, conserve XP/niveaux/monnaie ; nouveaux champs : `Purchases`, `Passes`, `Quests`, `Daily`, `Loadouts`, `Cosmetics{Owned, Equipped}`, `Rank`, `Stats`), signal `ProfileLoaded`.
7. `Bootstrap.server.luau` / `Bootstrap.client.luau` ; services `SettingsService` (keybinds), `ProgressionService` (niveau joueur), `CurrencyService`, `JutsuService` (pipeline K1 réécrit, effets encore serveur dans cette phase), `EnemyService` (mannequins, hook par table et non par attribut, effacement de `LastAttackerUserId`), `AntiCheatService` (compteur d'abus + kick), `Log`.
...
1. `Pure/ComboResolver.luau` : `new(recipes, maxLength)`, `push(state, element) → {action = "cast"|"wait"|"reset", jutsuId?}`, `timeout(state)`, index préfixe ; tests (préfixes, timeouts, longueur max, ambiguïtés, spam d'une touche).
2. `CombatConfig` : chakra (max, coût par jutsu, régénération hors/en combat, délai « en combat »), M1 (3 coups + finisher knockback, fenêtres), dash (distance, i-frames, cooldown), block (réduction, jutsus qui cassent la garde), stun/ragdoll léger, i-frames par cible par jutsu, tolérance de latence.
3. `CombatService` : `ApplyDamage(source, target, amount, kind, tags)` unique (gate PvP par mode/équipe/spawn-protection/ForceField, i-frames, block, stun, knockback serveur via `LinearVelocity` éphémère), `CombatState` par joueur (chakra, inCombat, blocking, stunned, iFrames), tick centralisé `Heartbeat`.
4. `JutsuService` réécrit : coût chakra, cooldowns, origine/direction serveur avec tolérance bornée, exécution via `JutsuEffects` serveur **sans visuel** (hit detection `GetPartBoundsInRadius/Box`, raycasts, Parts de collision invisibles, plus de `Touched`), XP conditionnée à ≥ 1 hit ou à un résultat de match.
5. Roster `JutsuConfig` ≥ 16 jutsus (8 conservés rééquilibrés + 8 nouveaux) sur 4 éléments + Foudre (4 combos), combos 2-4 touches, archétypes variés, arbre de combos sans préfixe bloquant pour les 2 touches de base (voir `GAME_DESIGN.md`), interactions élémentaires simples.
...
7. `CombatController` client : intentions `Melee`, `Dash`, `Block` ; HUD chakra.

Tests Phase 2 : `ComboResolver`, `JutsuConfig` (unicité, longueurs, coûts > 0, `Unlock` valide, tous les `vfxId` existent dans `VfxConfig`).

## Phase 3 — Mobile, manette, UI responsive, localisation complète

1. `InputController` : `ContextActionService` pour éléments / Dash / Melee / Block / Menu, boutons tactiles pouce droit (icônes + couleurs par élément), haptique si dispo, manette (D-pad = éléments, boutons = actions), clavier QWERTY/AZERTY sans conflit, rebind clavier **et** manette dans les options.
...
4. `Pure/Elo.luau` (K dégressif, symétrie, bornes, tests) ; `Pure/RankTiers.luau` (Genin → Chunin → Jonin → Anbu → Kage) ; `RankingService` + `LeaderboardService` (`OrderedDataStore` par saison : global, 1v1, 3v3 ; refresh 60 s sous budget) ; `MatchController` + `LeaderboardScreen`.
...
Déblocage de jutsus par niveau, loadouts (slots limités), `Pure/QuestLogic.luau` + `QuestService` (journalières / hebdomadaires data-driven), `Pure/DailyStreak.luau` + `DailyRewardService`, battle pass saisonnier gratuit/premium, `CosmeticService` (skins de jutsu = palettes VFX, auras, traînées, effets de kill, titres ; rendu par `VfxController`), récompenses de fin de saison.

Tests Phase 6 : `QuestLogic`, `DailyStreak`, `BattlepassConfig` (50 paliers, deux pistes).

## Phase 7 — Monétisation, analytics, économie

`MonetizationConfig` (IDs + validation au boot avec `warn` par ID manquant), `MonetizationService` (game passes : VIP, slots de loadout, Foudre, pack de skins ; dev products : 3 packs de Ryo, pass premium, saut de paliers, boost XP ; ownership vérifiée à la connexion / cachée / mise à jour sur `PromptGamePassPurchaseFinished` ; `Pure/ReceiptProcessor.luau` idempotent testé avec mocks ; achats hors connexion), `ShopService` + `Pure/ShopRotation.luau` (rotation quotidienne seedée), bonus Premium, `PolicyService` avant tout aléatoire payant (boutique directe, pas de caisses), prompts contextuels non agressifs (jamais dans les 3 premières minutes de la première session), `AnalyticsService` (économie, funnel, custom, rate-limité), `docs/ECONOMY.md`.
